...
class contactos
 {
    ...
    public function mostrar()
    {
        ...
        if($result->num_rows >0)
        {
            while($row = $result->fetch_assoc())
            {
                ...
                <td>' .$row ["correo"].'</td>
                <td>' .$row ["nota"].'</td>
                <td><button class="btn btn-primary" onclick="actualizar(\'' .$row["id"] . '\')"><img src="img/lapiz.png" width="30px"></button>
                </td>
                </tr>';
            }
        }
    ...
    public function mostrar2()
    {
        ...
        if($result->num_rows >0)
        {
            while($row = $result->fetch_assoc())
            {
                ...
                <td>' .$row ["correo"].'</td>
                <td>' .$row ["nota"].'</td>
                <td><button class="btn btn-danger" onclick="eliminarItem(\'' .$row["id"] . '\')"><img src="img/basura.png" width="30px"></button></td>
                </tr>';
            }
        }
        $tabla.='</tbody>
    ...
    function mostrar3()
    {
        ...
        if($result ->num_rows >0)
        {
            ...
            {
                $tabla .='<tr>
                <td><img width="30" src="http://localhost/App/' . $row["foto"].'" class="foto" alt=""></td>
                <td>' .$row ["contacto"].'</td>
                <td>' .$row ["puesto"].'</td>
                <td>' .$row["empresa"].'</td>
                <td>' .$row ["tipo"].'</td>
     ...
     public function mostrar4()
    {
        ...
        if($result->num_rows >0)
        {
            while($row = $result->fetch_assoc())
            {
                ...
                <td>' .$row ["correo"].'</td>
                <td>' .$row ["nota"].'</td>
                <td><button class="btn btn-outline-info" onclick="generaword(\'' .$row["id"] . '\')"><img src="img/word.png" width="30px"></button></td>
                </tr>';
            }
        }
        $tabla.='</tbody>
